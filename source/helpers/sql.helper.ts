...
import { DB_CONNECTION_STRING_SHOP, Queries } from "../constants";
...
export class SqlHelper {
    ...
    public static createNew(errorService: ErrorService, query: string, original: entityWithId, ...params: (string | number | undefined)[]): Promise<entityWithId> {
    ...
    public static getStringFromUser(stringFromUser : string): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            resolve(stringFromUser);
        })
    }

    private static openConnection(errorService: ErrorService): Promise<Connection> {
        return new Promise<Connection>((resolve, reject) => {
            SqlHelper.sql.open(DB_CONNECTION_STRING_SHOP, (connectionError: Error, connection: Connection) => {
                if (connectionError) {
